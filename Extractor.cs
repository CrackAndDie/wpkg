    ...
    class Extractor
    {
        ...
        public static void ExtractEverything(Stream DebianPackageStream, string WorkDir) {
            ...
            while (DebianPackage.Read()) {
				...
				else if (DebianPackage.FileName == "control.tar.xz")
				{
					ExtractContent(DebianPackage, 4);
				}
				else if (DebianPackage.FileName == "data.tar") {
                    ExtractContent(DebianPackage, 3);
                }
				else if (DebianPackage.FileName == "data.tar.xz")
				{
					ExtractContent(DebianPackage, 5);
				}
			}
        }
        ...
        private static void ExtractContent(ARFile DebFile, int variant) {
			...
			else if (variant == 4) /*control.tar.xz*/
			{
				fn = "control.tar.xz";
				fdir = OutPath + "\\" + DebName + "_control.tar.xz_CONTENT";
		    } else if (variant == 3) /*data.tar*/ {
                fn = "data.tar";
                fdir = OutPath + "\\" + DebName + "_data.tar_CONTENT";
            }
			else if (variant == 5) /*data.tar.xz*/
			{
				fn = "data.tar.xz";
				fdir = OutPath + "\\" + DebName + "_data.tar.xz_CONTENT";
			}
